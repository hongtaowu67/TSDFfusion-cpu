...
//    Add in plane segmentation and euclidean clustering to segment the object from a plane
...
int main(int argc, char *argv[])
{
    ...
    float trunc_margin = voxel_size * 8;
    ...
    float *filterred_voxel_grid_TSDF = new float[voxel_grid_dim_x * voxel_grid_dim_y * voxel_grid_dim_z];
    float *voxel_grid_weight = new float[voxel_grid_dim_x * voxel_grid_dim_y * voxel_grid_dim_z];
    for (int i = 0; i < voxel_grid_dim_x * voxel_grid_dim_y * voxel_grid_dim_z; ++i)
    {
        voxel_grid_TSDF[i] = 1.0f;
        filterred_voxel_grid_TSDF[i] = 1.0f;
    }
    ...
    std::cout << "Saving surface point cloud (tsdf_raw.ply)..." << std::endl;
    SaveVoxelGrid2SurfacePointCloud("model/tsdf_raw.ply", voxel_grid_dim_x, voxel_grid_dim_y, voxel_grid_dim_z,
                                    voxel_size, voxel_grid_origin_x, voxel_grid_origin_y, voxel_grid_origin_z,
                                    voxel_grid_TSDF, voxel_grid_weight, 0.2f, 0.0f);
    std::cout << "Finish saving the tsdf_raw..." << std::endl;

    // Read the saved ply
    pcl::PointCloud<pcl::PointXYZ>::Ptr pc(new pcl::PointCloud<pcl::PointXYZ>);
    pcl::PointCloud<pcl::PointXYZ>::Ptr pc_plane_rm(new pcl::PointCloud<pcl::PointXYZ>);
    pcl::PointCloud<pcl::PointXYZ>::Ptr pc_eu_cluster(new pcl::PointCloud<pcl::PointXYZ>);

    pcl::PLYReader reader;
    reader.read<pcl::PointXYZ>("model/tsdf_raw.ply", *pc);
    ...
    SaveVoxelGrid2SurfacePointCloud("model/tsdf.ply", voxel_grid_dim_x, voxel_grid_dim_y, voxel_grid_dim_z,
                                    voxel_size, voxel_grid_origin_x, voxel_grid_origin_y, voxel_grid_origin_z,
                                    filterred_voxel_grid_TSDF, voxel_grid_weight, 0.2f, 0.0f);

    // Save TSDF voxel grid and its parameters to disk as binary file (float array)
    std::cout
        << "Saving TSDF voxel grid values to disk (tsdf.bin)..." << std::endl;
    ...
    for (int i = 0; i < voxel_grid_dim_x * voxel_grid_dim_y * voxel_grid_dim_z; ++i)
    {
        outFile.write((char *)&filterred_voxel_grid_TSDF[i], sizeof(float));
    }
    outFile.close();
